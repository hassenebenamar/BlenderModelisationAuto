...
# Constants
NUM_RENDERS = 35
OUTPUT_DIR = 'D:/Polytech/5A/TAI/output/Fourth Batch/kitchen_light_change'
...
constraints = {
    ...
    'scratching_post_001': (0.15, 0.5, 0),
    #'fridge_001': (0, 2, 0),
    'SIDE_BOARD': (0, 1.1, 0)
}
...
for render_count in range(NUM_RENDERS):
    ...
    for obj in bpy.data.objects:
        if obj.name != REFERENCE_OBJECT_NAME:
            # Exclude objects like Camera from color fetching
            if obj.type == 'LIGHT':
    ...
    render_file_path = os.path.join(OUTPUT_DIR, f'kitchen_render_{render_count}_lightPower_{light_power}.png')
    bpy.context.scene.render.filepath = render_file_path
    bpy.ops.render.render(write_still=True)

    # Write positions to a JSON file
    json_file_path = os.path.join(OUTPUT_DIR, f'kitchen_render_{render_count}_lightPower_{light_power}_positions.json')
