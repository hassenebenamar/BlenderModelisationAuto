...
# Constants
NUM_RENDERS = 3
OUTPUT_DIR = 'D:/Polytech/5A/TAI/output/Fifth Batch/bedroom_sofa_change_light_change'
...
initial_colors = {obj.name: get_initial_object_color(obj) for obj in bpy.data.objects if
...
for render_count in range(NUM_RENDERS):
    ...
    if sofa_obj:
        ...
        specific_color = generate_random_color()
        change_object_color(sofa_obj, specific_color)
        randomize_object_location_non_symmetric(sofa_obj, -0.35, 0.75, 0, 0, 0, 0)
        positions_and_directions["sofa_001"] = sofa_obj.location
        # 'sofa_001': (0.35, 0, 0)
    ...
    render_file_path = os.path.join(OUTPUT_DIR, f'bedroom_render_{render_count}_lightPower_{light_power}.png')
    bpy.context.scene.render.filepath = render_file_path
    bpy.ops.render.render(write_still=True)

    # Write positions to a JSON file
    json_file_path = os.path.join(OUTPUT_DIR, f'bedroom_render_{render_count}_lightPower_{light_power}_positions.json')
